...
const main = async (receiverAddress: string) => {
  const rpcEndpoint =
    ...
    "https://api-mainnet-beta.renec.foundation:8899"; // https://api-testnet.renec.foundation:8899

  const network =
    rpcEndpoint == "https://api-mainnet-beta.renec.foundation:8899"
      ? "mainnet"
      : "testnet";
  ...
  // reusd
  const reusdMintTokenAccount = new PublicKey(
    process.env.REUSD_TOKEN || ""
  );
  ...
  // prop
  const propMintTokenAccount = new PublicKey(
    process.env.PROP_TOKEN || ""
  );
  ...
  // airdrop reusd
  const payerReusdATA = await Token.getAssociatedTokenAddress(
    ASSOCIATED_TOKEN_PROGRAM_ID,
    TOKEN_PROGRAM_ID,
    reusdMintTokenAccount,
    payer.publicKey,
  ...
  if (receiverReusdATAInfo == null) {
    ...
    if (receiverReusdBalance && receiverReusdBalance >= reusdMaximumUiAmount) {
  ...
  // airdrop prop
  const payerPropATA = await Token.getAssociatedTokenAddress(
    ASSOCIATED_TOKEN_PROGRAM_ID,
    TOKEN_PROGRAM_ID,
  ...
  console.log(
    `https://explorer.renec.foundation/tx/${txSig}?cluster=${network}`
  );
};

try {
  main("7ZKBj6TTkB2ZH3NnagjbfC4YpXFrMjv9jd2MHJnJWv6k");
} catch (error) {
  console.log(error);
}
